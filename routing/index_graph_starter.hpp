...
public:
  ...
  void GetEdgeList(JointSegment const & parent, Segment const & current, bool isOutgoing,
                   std::vector<JointEdge> & edges, std::vector<RouteWeight> & parentWeights) const
  {
    return m_graph.GetEdgeList(parent, current, isOutgoing, edges, parentWeights);
  }
