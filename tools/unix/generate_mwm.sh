...
GENERATE_EVERYTHING="--generate_features --generate_geometry --generate_index --dump_cities_boundaries --cities_boundaries_data=$CITIES_BOUNDARIES_DATA"
...
if [ "$SOURCE_TYPE" == "o5m" ]; then
  ...
  $GENERATOR_TOOL $INTDIR_FLAG --data_path="$TARGET" --user_resource_path="$DATA_PATH" --make_cross_mwm ${GENERATE_CAMERA_SECTION-} --make_routing_index --generate_traffic_keys --output="$BASE_NAME"
else
